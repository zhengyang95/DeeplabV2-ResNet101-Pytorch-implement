...
def test(config_path, model_path, cuda, log_dir):
    ...
    raw_out_png_path = os.path.join(save_dir, 'raw/png')
    raw_out_npy_path = os.path.join(save_dir, 'raw/npy')

    os.makedirs(raw_out_png_path, exist_ok=True)
    os.makedirs(raw_out_npy_path, exist_ok=True)
    ...
    for image_ids, images, gt_labels, cls_labels in tqdm(
        ...
        for image_id, logit in zip(image_ids, logits):
            filename = os.path.join(raw_out_npy_path, image_id + ".npy")
        ...
        for image_id in image_ids:
            filename = os.path.join(raw_out_png_path, image_id + ".png")
            cv2.imwrite(filename, labels[0].cpu().numpy())
...
#     "--log_dir", required=True, help="tensorboard log dir"
# )
def crf(config_path, n_jobs, log_dir, save_img):
    """
    ...
    makedirs(save_dir)
    raw_out_npy_path = os.path.join(save_dir, 'raw/npy')
    crf_out_png_path = os.path.join(save_dir, 'crf/png')
    crf_out_npy_path = os.path.join(save_dir, 'crf/npy')

    os.makedirs(crf_out_png_path, exist_ok=True)
    os.makedirs(crf_out_npy_path, exist_ok=True)
    ...
    def process(i):
        ...
        filename = os.path.join(crf_out_png_path, image_id + ".png")
...
if __name__ == "__main__":
    ...
    # for val
    config_path = '/utilisateurs/lyuzheng/DeepL/FSSS/DRS-main/DeepLab-V2-PyTorch/configs/voc12.yaml'
    model_path = '/media/data/lyuzheng/Result/2024/08/0827_1_dlv2_gt/deeplabv2_resnet101_msc/train_aug_cls/DLV2_Res101_seg.pth'
    cuda = 'True'

    # for CRF
    n_jobs = 12
    log_dir = '/media/data/lyuzheng/Result/2024/08/0827_1_dlv2_gt/deeplabv2_resnet101_msc/train_aug_cls/'

    test(config_path, model_path, cuda, log_dir)
    # crf(config_path, n_jobs, log_dir, save_img=True)